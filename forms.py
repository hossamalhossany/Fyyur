...
class ArtistForm(FlaskForm):
    ...
    def validate_phone_length(field):
        if len(field.data) != 10:
            raise ValidationError('Phone Number must be 10 number !!! ')
        try:
            phone_number = phonenumbers.parse(field.data)
            if not (phonenumbers.is_valid_number(phone_number)):
                raise Exception('Invalid Number !!! ')
        finally:
            raise Exception('again enter the number ')

    phone = StringField(
        # TODO implement validation logic for state
        'phone',
        validators=[DataRequired(), validate_phone_length]
    )
